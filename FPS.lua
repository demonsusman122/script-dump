...
if UniversalAimbotLoaded == true then
    Notify("MoonWare", "Script Loaded Already", "", 3)
    return 
end
    ...
    if workspace and workspace:FindFirstChild"Teams" then 
        if getgenv().TeamCheck == true then
            if #workspace.Teams:GetChildren() == 0 then 
                ...
                SeparateNotify("MoonWare", "TeamCheck set to: "..tostring(getgenv().TeamCheck).." because there are no teams!", "", 3)
            end
        end
    end
end
        ...
        if Key.UserInputType == Enum.UserInputType.MouseButton2 then 
            pcall(function()
                ...
                if Target ~= nil then 
                    ...
                    Notify("MoonWare", "Aimlock Target: "..tostring(AimlockTarget), "", 3)
                end
            end)
        end
        if Key.KeyCode == Enum.KeyCode[AimlockToggleKey] then 
            Aimlock = not Aimlock
            Notify("MoonWare", "Aimlock: "..tostring(Aimlock), "", 3)
        end
    end
end)
